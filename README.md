...
rsync -a \
    ...
    --exclude='*.zip' \
    --exclude='*.tar.gz' \
    ./ build/notice-tracker/
```

Point Plugin Check (and the wp.org SVN/zip submission) at `build/notice-tracker/`, not at the dev folder. The `build/` directory is git-ignored.
