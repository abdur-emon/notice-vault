<?php
/**
 * Plugin Name:       Quietboard Notice Manager
 * Plugin URI:        https://github.com/abdur-emon/quietboard-notice-manager
 ...
 * Text Domain:       quietboard-notice-manager
...
Quietboard_Notice_Manager\Core\Autoloader::register();
...
function wpnm_activate_plugin() {
	...
	Quietboard_Notice_Manager\Core\Activator::activate();
}
...
function wpnm_deactivate_plugin() {
	...
	Quietboard_Notice_Manager\Core\Deactivator::deactivate();
}
 ...
 * `quietboard-notice-manager`). A manual call to load_plugin_textdomain() is no longer
 * necessary and is flagged as a discouraged function by Plugin Check, so we
 * rely on the auto-loader.
 */
function wpnm_init_plugin() {
	$plugin = Quietboard_Notice_Manager\Core\Plugin::get_instance();
	$plugin->run();
}
